    ...
    uint64_t* enclave_addr)
{
    ...
    if (context->create_zero_base_enclave)
    {
        ...
        /*
         * elrange_size is the size of the enclave address range.
         */
        enclave_elrange.elrange_size =
            oe_round_u64_to_pow2(enclave_size + (uint64_t)start_address);

        ex_features = OE_ENCLAVE_CREATE_EX_EL_RANGE;
        ex_features_array[OE_ENCLAVE_CREATE_EX_EL_RANGE_BIT_IDX] =
            &enclave_elrange;
    }
    else
